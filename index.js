...
async function processMorningNews(env) {
  ...
  const issueTitle = `每日早报 ${new Date().toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai' })}`;
...
async function uploadImageToSMMS(imageUrl, smmsToken = '') {
  try {
    ...
    formData.append('smfile', new Blob([imageBuffer]), 'news.jpg');
    
    // 上传到SM.MS
    const uploadResponse = await fetch('https://sm.ms/api/v2/upload', {
      method: 'POST',
      headers: {
        'Authorization': smmsToken || undefined,
      },
      body: formData
    });
